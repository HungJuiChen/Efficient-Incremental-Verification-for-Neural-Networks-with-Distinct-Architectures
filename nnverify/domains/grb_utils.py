...
def check_optimization_success(model, opt_var, input_vars):
    ...
    #     return False, None
    elif model.status in (grb.GRB.INFEASIBLE, grb.GRB.INF_OR_UNBD):
        try:                               # IIS is only a nice-to-have
            model.computeIIS()
            model.write("model.ilp")
        except grb.GurobiError as e:       # model turned out to be feasible
            print(f"[warning] IIS skipped: {e}")
        return False, None
    else:
        print('\n')
