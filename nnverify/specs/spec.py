...
def get_specs(dataset, spec_type=InputSpecType.LINF, eps=0.01, count=None):
    ...
    #add by hungjui2
    elif dataset == Dataset.TINY2:
        # Build `count` many 2-D L∞ box specs for the tiny net
    #     if count is None:
    #         count = 1
    ...
    #     C = torch.tensor([[1.0]])          # 1×1 constraint for the 1-output net
    ...
    #     # one output-constraint shared by all clauses
    #     out_constr = Constraint(OutSpecType.LOCAL_ROBUST, label=dummy_label)
    #     out_constr.constr_mat = (C, 0)

    #    # use PATCH so Property can store multiple InputSpec clauses
    #     prop = Property(ilbs, iubs, InputSpecType.PATCH,
    #                     out_constr, dataset, input=None)
    #     return [prop], None          # list with exactly one Property
        x        = torch.zeros(2)
        ilb, iub = x - eps, x + eps

        dummy_label = torch.tensor(0, dtype=torch.int64)
        #C = torch.tensor([[1.0]])           # 1×1 output-constraint matrix
        C = torch.zeros((1, 1))

        out_constr = Constraint(OutSpecType.LOCAL_ROBUST, label=dummy_label)
        out_constr.constr_mat = (C, 0)      # overwrite the default 10×9 matrix

        prop = Property(ilb, iub, InputSpecType.LINF, out_constr, dataset, input=x)
        return [prop], None                 # list of length 1
    ###
    else:
...
### added by hungjui2
# ------------------------------------------------------------------
def reindex(self, layer_maps):
    """
    Translate neuron indices according to `layer_maps`,
    a list such that layer_maps[i] is {new_idx → old_idx} for layer i.
    """
    if self.relu_spec is None or not layer_maps:
            return self

    new_mask = {}
    for (layer, old_idx), val in self.relu_spec.relu_mask.items():
        if layer >= len(layer_maps):           # layer unchanged
            new_mask[(layer, old_idx)] = val
            continue
        for new_idx, mapped_old in layer_maps[layer].items():
            if mapped_old == old_idx:
                new_mask[(layer, new_idx)] = val
                break
    self.relu_spec.relu_mask = new_mask
    return self
    # ------------------------------------------------------------------
