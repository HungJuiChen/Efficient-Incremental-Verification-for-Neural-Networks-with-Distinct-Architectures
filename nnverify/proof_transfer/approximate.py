...
class Quantize:
    ...
    def approximate(self, net_name, dataset, should_check_accuracy = True, conv = False, dummy=True):
        ...
        # Revised by hungjui2
        # Load the network (ONNX or torch)
        net = util.get_net(net_name, dataset)
        # If it's an ONNX‐parsed Network, just return it (skip quantization)
        if getattr(net, 'net_format', None) == 'onnx':
            return net
        # Otherwise we have a true torch model inside .torch_net
        if getattr(net, 'torch_net', None) is not None:
            net.torch_net.eval()
        ### End
