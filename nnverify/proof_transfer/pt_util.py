...
def plot_line_graph(pt_args, res, res_pt):
    ...
    ### add by hungjui2
    file_name = res.get_plot_file_name(pt_args)

    # Save inside the sub-folder (as before) …
    plt.savefig(dir_name + file_name)
    # … and **also** at the location the unit-test expects
    plt.savefig(common.RESULT_DIR + file_name)
    plt.close('all')
