...
class BnB:
    ...
    def get_init_specs(self, init_prop):
        ### added by hungjui2
        if (hasattr(self.args, "neuron_map") and self.args.neuron_map is not None
                and hasattr(init_prop, "reindex")):          # ← new guard
            init_prop = init_prop.reindex(self.args.neuron_map)
        ### --------------------
