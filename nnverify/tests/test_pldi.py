...
class TestRandom(TestCase):
    def test1(self):
        args = pt.TransferArgs(net=config.MNIST_FFN_L2, domain=Domain.LP, approx=ap.Random(0.02),
                               dataset=Dataset.MNIST, eps=0.02,
    ...
    def test2(self):
        args = pt.TransferArgs(net=config.MNIST_FFN_L2, domain=Domain.LP, approx=ap.Random(0.05),
                               dataset=Dataset.MNIST, eps=0.02,
                               split=Split.RELU_ESIP_SCORE, count=COUNT, pt_method=IVAN(0, 0.003),
                               timeout=SMALL_TIMEOUT)
        pt.proof_transfer(args)
    ...
    def test3(self):
        args = pt.TransferArgs(net=config.MNIST_FFN_01, domain=Domain.LP, approx=ap.Random(0.02),
                               dataset=Dataset.MNIST, eps=0.1,
                               split=Split.RELU_ESIP_SCORE, count=COUNT, pt_method=IVAN(0, 0.007),
                               timeout=SMALL_TIMEOUT)
        pt.proof_transfer(args)
    ...
    def test4(self):
        args = pt.TransferArgs(net=config.MNIST_FFN_01, domain=Domain.LP, approx=ap.Random(0.05),
                               dataset=Dataset.MNIST, eps=0.1,
                               split=Split.RELU_ESIP_SCORE, count=COUNT, pt_method=IVAN(0, 0.007),
                               timeout=SMALL_TIMEOUT)
        pt.proof_transfer(args)
    ...
    def test5(self):
        args = pt.TransferArgs(net=config.CIFAR_CONV_SMALL, domain=Domain.LP,
                               approx=ap.Random(0.02), dataset=Dataset.CIFAR10, eps=2 / 255,
                               split=Split.RELU_ESIP_SCORE, count=COUNT, pt_method=IVAN(0, 1e-4), timeout=SMALL_TIMEOUT)
        pt.proof_transfer(args)

    def test6(self):
        args = pt.TransferArgs(net=config.CIFAR_CONV_SMALL, domain=Domain.LP,
                               approx=ap.Random(0.05), dataset=Dataset.CIFAR10, eps=2 / 255,
                               split=Split.RELU_ESIP_SCORE, count=COUNT, pt_method=IVAN(0, 1e-4), timeout=SMALL_TIMEOUT)
        pt.proof_transfer(args)

    def test7(self):
        args = pt.TransferArgs(net=config.CIFAR_OVAL_WIDE, domain=Domain.LP,
                               approx=ap.Random(0.02), dataset=Dataset.CIFAR10, eps=4 / 255,
                               split=Split.RELU_ESIP_SCORE, count=COUNT, pt_method=IVAN(0, 1e-4), timeout=BIG_TIMEOUT)
        pt.proof_transfer(args)

    def test8(self):
        args = pt.TransferArgs(net=config.CIFAR_OVAL_WIDE, domain=Domain.LP,
                               approx=ap.Random(0.05), dataset=Dataset.CIFAR10, eps=4 / 255,
                               split=Split.RELU_ESIP_SCORE, count=COUNT, pt_method=IVAN(0, 1e-4), timeout=BIG_TIMEOUT)
        pt.proof_transfer(args)
